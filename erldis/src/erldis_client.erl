...
%% of From pids in State.calls, then calls gen_server3:reply when it receives
...
subscribe(Client, Cmd, Class, Pid)->
	case gen_server3:call(Client, {subscribe, Cmd, Class, Pid}, ?default_timeout) of
		{error, Reason} -> throw({error, Reason});
		Retval -> Retval
	end.

unsubscribe(Client, Cmd, Class)->
	case gen_server3:call(Client, {unsubscribe, Cmd, Class}, ?default_timeout) of
		{error, Reason} -> throw({error, Reason});
		Retval -> Retval
	end.
...
send(Client, Cmd, Timeout) ->
	Piped = gen_server3:call(Client, is_pipelined),
	
	if
		Piped ->
			gen_server3:cast(Client, {send, Cmd});
		true ->
			case gen_server3:call(Client, {send, Cmd}, Timeout) of
				{error, Reason} -> throw({error, Reason});
				Retval -> Retval
			end
	end.
...
start(Host, Port, Options, false) ->
	...
	% server is shutdown
	gen_server3:start(?MODULE, [Host, Port, false], Options);
start(Host, Port, Options, true) ->
	gen_server3:start_link(?MODULE, [Host, Port, true], Options).
...
stop(Client) -> gen_server3:call(Client, disconnect).
...
handle_call(get_all_results, From, #redis{pipeline=true, calls=Calls}=State) ->
	case queue:len(Calls) of
		...
		_ ->
			...
			R = fun(V) -> gen_server3:reply(From, V) end,
...
terminate(_Reason, State) ->
	...
	R = fun(From) -> gen_server3:reply(From, {error, closed}) end,
